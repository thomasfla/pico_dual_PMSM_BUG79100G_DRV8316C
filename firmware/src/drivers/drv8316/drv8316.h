...
class DRV8316Driver {
	...
	public:
		DRV8316Driver(int cs, bool currentLimit = false, int nFault = NOT_SET) : currentLimit(currentLimit), cs(cs), nFault(nFault), spi(&SPI), settings(DRV8316_SPI_HZ, MSBFIRST, SPI_MODE1) {};
		...
		uint16_t readRegisterRaw(uint8_t addr);
	...
	private:
		...
		uint8_t control1Shadow = 0;
		uint8_t control2Shadow = 0;
		uint8_t control3Shadow = 0;
		uint8_t control4Shadow = 0;
		uint8_t control5Shadow = 0;
		uint8_t control6Shadow = 0;
		uint8_t control10Shadow = 0;
};
